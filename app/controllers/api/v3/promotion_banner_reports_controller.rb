    ...
    class PromotionBannerReportsController < ApiController
      ...
      def index
        query = "SELECT p.id as \"promotion_id\", c.title, p.paid, date_trunc('day', pbr.report_date) AS \"report_date\",
          ...
          (
            SELECT SUM(impression_count) FROM promotion_banner_reports WHERE promotion_banner_id = pb.id AND date_trunc('day', report_date + INTERVAL '5 hours') < CURRENT_DATE
          ) as total_impression_count,
          (
            SELECT SUM(click_count) FROM promotion_banner_reports WHERE promotion_banner_id = pb.id AND date_trunc('day', report_date + INTERVAL '5 hours') < CURRENT_DATE
          ...
          WHERE date_trunc('day', pbr.report_date + INTERVAL '5 hours') < CURRENT_DATE
