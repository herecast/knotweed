...
RSpec.describe SubscriptionsMailchimpClient do
  ...
  describe '#lists' do
    ...
    let!(:apistub1) do
      ...
      ).to_return(body: { lists: %w[list1 list2] }.to_json)
    end
    ...
    it 'returns all the lists, regardless of pagination' do
      expect(subject.lists.to_a).to eq %w[list1 list2 list3]
    end
  end
  ...
  describe '#update_campaign' do
    ...
    it 'executes without exception' do
      expect do
        subject.update_campaign(campaign_identifier: 'qwer',
                                subject: 'sub',
                                title: 'tle',
                                from_name: 'from',
                                reply_to: 'reply')
  ...
  describe '#create_content' do
    let!(:apistub) do
      stub_request(:put,
                   "#{endpoint}/campaigns/qwer/content").with(
                     basic_auth: auth,
                     headers: {
                       'Content-Type' => 'application/json',
                       'Accept' => 'application/json'
